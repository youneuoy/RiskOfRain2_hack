...
{
	if (menuMainShow || menuCameraSettingsShow || menuCharacterSettingsShow) // ĺńëč âűçűâŕĺňń˙ őĺíäëĺđ
	{
		//check window messages
		checkMsgs(hwnd, uMsg, wParam, lParam);
		
		ImGui_ImplWin32_WndProcHandler(hwnd, uMsg, wParam, lParam);
	}
	return CallWindowProc(oWndProc, hwnd, uMsg, wParam, lParam); // ĺńëč ćĺ íĺň, âűçűâŕĺě îđčăčíŕëüíűé ďđîöĺńń
}
